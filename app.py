...
# Decorator to check if in read-only mode
...
class Reservation(db.Model):
    ...
    platform = db.Column(db.String(20), nullable=False)  # 'booking' or 'airbnb'
...
def index():
    ...
    today = date.today()  # DODAJ OVU LINIJU
    return render_template('index.html', reservations=reservations, is_readonly=is_readonly, today=today)  # DODAJ today=today
...
def calendar_view():
    reservations = Reservation.query.order_by(Reservation.check_in).all()
    is_readonly = session.get('readonly_mode', False)
    return render_template('calendar.html', reservations=reservations, is_readonly=is_readonly)
